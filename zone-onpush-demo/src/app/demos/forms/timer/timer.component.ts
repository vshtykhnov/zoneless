...
@Component({
  ...
  template: `
    <div #container class="timer-block">
      <p>TimerComponent (OnPush){{ isRefreshPhase ? flash() : '' }}</p>
...
export class TimerComponent implements DoCheck, OnDestroy {
  ...
  startTimer() {
    ...
    this.timerInterval = setInterval(() => {
      this.timerCount++;
      console.log(
        'TimerComponent timer tick (Zone + OnPush):',
        this.timerCount
      );
  ...
  throwException() {
    setTimeout(() => {
      throw new Error(
        'TimerComponent: Exception thrown from setTimeout (Zone + OnPush)'
      );
    }, 0);
  }

  ngDoCheck() {
    console.log('🔄 TimerComponent change detection (Zone + OnPush)');

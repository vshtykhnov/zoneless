...
@Component({
  ...
  template: `
    <div class="counter-block parent">
      <p>Parent counter (Zone + OnPush, updates every 1s): {{ counter }}</p>
...
export class CounterParentComponent implements DoCheck, OnDestroy {
  ...
  constructor(private cdr: ChangeDetectorRef) {
    // В zone + onpush режиме setInterval НЕ запускает автоматическую детекцию изменений
    this.intervalId = setInterval(() => {
      this.counter++;
      console.log('counter', this.counter);
      // Ручной запуск детекции изменений в zone + onpush режиме
      this.cdr.detectChanges();
    }, 1000);
  }

  ngDoCheck() {
    console.log('CounterParentComponent change detection (Zone + OnPush)');
